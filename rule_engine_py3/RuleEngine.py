...
class RuleEngine(object):
    ...
    def _loadRules(self, rule_dir="./rules"):
        print ("\n----- _loadRules")
        tmpdir = rule_dir
        if not tmpdir.endswith('/'):
            tmpdir += '/'
    ...
    def extract(self, input_str = ''):
        ...
        te = []
        ct=0
        for key,vals in self._rules.items():
            #print ("total %s rules for %s" % (str(len(vals)),key) )
            for val in vals:
                ct=ct+1
                #print  (" rule %s : %s"% (ct,val) )
                for m in re.finditer(val[0], input_str):
                    '''
                    if ((m.start() > 0) and (input_str[m.start()-1] not in self._seperators )) or ((m.end() < len(input_str)) and (input_str[m.end()] not in self._seperators )):
